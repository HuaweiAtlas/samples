...
/* global variable for holding inotify file descriptor */
int g_iMboxInotifyFd = -1;
/* global map for mapping watch descriptor to docker name string */
std::unordered_map<int, string> g_MboxWd2Docker;
/* global map for mapping docker name string to watchdescriptor */
...
int mbox_write_msg(const char *msg_file, const char *msg, unsigned int size)
{
    ...
    /* first we shall try lock the mailbox file */
...
struct inotify_event * mbox_get_event(struct inotify_event *event, unsigned int *offset, unsigned int *end)
{
    ...
    memset_s(event, MBOX_READ_EVENT_MAX * sizeof(struct inotify_event), 0, *end * sizeof(struct inotify_event));
    *offset = 0;
    *end = 0;
    ...
    *end = iRet / sizeof(struct inotify_event);
    ...
    {
        printf("Strange, read bytes[%d] not multiple of inotify_event size[%d]", iRet, (int)sizeof(struct inotify_event));
        //return -1;
    }
